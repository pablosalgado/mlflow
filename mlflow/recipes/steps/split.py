        ...
        if not self.skip_data_profiling:
            ...
            if self.positive_class:
                ...
                positive_negative_profile = get_pandas_data_profiles(
                    [
                        ["Positive", positive_df.reset_index(drop=True)],
                        ["Negative", negative_df.reset_index(drop=True)],
                    ]
                )

                # Tab #4: data profiles positive negative training split.
                card.add_tab("Positive Negative Training Splits", "{{PROFILE}}").add_pandas_profile(
                    "PROFILE", positive_negative_profile
                )
